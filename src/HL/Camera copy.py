...
class Camera:
    ...
    def is_running_in_reversed(self, image = None, LEFT_IS_GREEN=True):
        ...
        average_red_index = np.mean(red_indices[red_indices > 0])  # Average index of red
        print(f"green: {average_green_index}, red: {average_red_index}")
        if LEFT_IS_GREEN and average_red_index > average_green_index :
            if log.getLogger().isEnabledFor(log.DEBUG):
                ...
                Image.fromarray(image).convert("RGB").save(os.path.join(DEBUG_DIR_wayfinding, f"wrong_direction_{self.debug_counter}.jpg"))
            return True
...
if __name__ == "__main__":
    ...
    image_path = "src\HL\wrong_direction48.jpg"  # Replace with your image path
    camera = Camera()
